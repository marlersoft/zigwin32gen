    ...
    return struct {
        fn WindowProc(hwnd: HWND, uMsg: u32, wParam: win32.WPARAM, lParam: win32.LPARAM) callconv(WINAPI) win32.LRESULT {
            ...
            if (uMsg == win32.WM_NCCREATE) {
                ...
                pThis = @as(*DERIVED_TYPE, @ptrCast(@alignCast(@alignOf(DERIVED_TYPE), pCreate.lpCreateParams)));
        ...
        pub fn Create(
            ...
            options: struct {
                dwExStyle: win32.WINDOW_EX_STYLE = @as(win32.WINDOW_EX_STYLE, @enumFromInt(0)),
            ...
            self.m_hwnd = win32.CreateWindowEx(options.dwExStyle, DERIVED_TYPE.ClassName(), lpWindowName, dwStyle, options.x, options.y, options.nWidth, options.nHeight, options.hWndParent, options.hMenu, win32.GetModuleHandle(null), @as(*anyopaque, @ptrCast(self)));
