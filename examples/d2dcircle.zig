...
fn MainWindowCreateGraphicsResources(self: *MainWindow) HRESULT {
    ...
    if (self.pRenderTarget == null) {
        ...
        const size = D2D_SIZE_U{ .width = @as(u32, @intCast(rc.right)), .height = @as(u32, @intCast(rc.bottom)) };

        hr = self.pFactory.?.ID2D1Factory_CreateHwndRenderTarget(&D2D1.RenderTargetProperties(), &D2D1.HwndRenderTargetProperties(self.base.m_hwnd.?, size),
        // TODO: figure out how to cast a COM object to a base type
        @as(*?*win32.ID2D1HwndRenderTarget, @ptrCast(&self.pRenderTarget)));

        if (SUCCEEDED(hr)) {
            const color = D2D1.ColorF(.{ .r = 1, .g = 1, .b = 0 });
            // TODO: how do I do this ptrCast better by using COM base type?
            hr = self.pRenderTarget.?.ID2D1RenderTarget_CreateSolidColorBrush(&color, null, @as(*?*win32.ID2D1SolidColorBrush, @ptrCast(&self.pBrush)));
...
fn MainWindowOnPaint(self: *MainWindow) void {
    ...
    if (SUCCEEDED(hr)) {
        ...
        self.pRenderTarget.?.ID2D1RenderTarget_FillEllipse(&self.ellipse, @as(*win32.ID2D1Brush, @ptrCast(self.pBrush)));
...
pub export fn wWinMain(_: HINSTANCE, __: ?HINSTANCE, ___: [*:0]u16, nCmdShow: u32) callconv(WINAPI) c_int {
    ...
    _ = win32.ShowWindow(win.base.Window(), @as(win32.SHOW_WINDOW_CMD, @enumFromInt(nCmdShow)));
...
fn MainWindowHandleMessage(self: *MainWindow, uMsg: u32, wParam: WPARAM, lParam: LPARAM) LRESULT {
    switch (uMsg) {
        win32.WM_CREATE => {
            ...
            if (FAILED(win32.D2D1CreateFactory(win32.D2D1_FACTORY_TYPE_SINGLE_THREADED, win32.IID_ID2D1Factory, null, @as(**anyopaque, @ptrCast(&self.pFactory))))) {
