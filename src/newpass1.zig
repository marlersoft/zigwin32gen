...
pub fn main() !u8 {
    ...
    for (api_list.items) |api_json_basename| {
        ...
        try jsonObjEnforceKnownFieldsOnly(root_obj, &.{
            "Constants", "Types", "Functions", "UnicodeAliases"
        }, api_json_basename);
        const types = (try jsonObjGetRequired(root_obj, "Types", api_json_basename)).array;
        try enumerateTypes(&type_map, api_json_basename, api_name_pool, types);
        const functions = (try jsonObjGetRequired(root_obj, "Functions", api_json_basename)).array;
        try enumerateFunctions(&type_map, &dll_jsons, api_json_basename, functions);
        dll_jsons.closeAll();
    }
...
fn enumerateTypes(
    ...
    for (types.items) |*type_node_ptr| {
        const type_obj = type_node_ptr.object;
        const kind = (try jsonObjGetRequired(type_obj, "Kind", json_basename)).string;
        const name_tmp = (try jsonObjGetRequired(type_obj, "Name", json_basename)).string;
        ...
        if (std.mem.eql(u8, kind, "ComClassID")) {
            ...
            // SomeComType.ClassID?
            //std.log.warn("TODO: put COM CLSID {s} alongside it's COM definition", .{name_tmp});
        } else if (std.mem.eql(u8, kind, "NativeTypedef")) {
            foundTypeDefinition(type_map, api, name_tmp);
            ...
            const type_ref: TypeRef = .{
                .api = api,
                .name = global_string_pool.add(name_tmp) catch |e| oom(e),
            };
            const t = getTypeFromRef(type_map, type_ref);
            t.definition_count += 1;
            global_com_types.append(allocator, type_ref) catch |e| oom(e);
            //std.log.info("COM type '{s}' in api {s}", .{name_tmp, api});
                //foundTypeDefinition(type_map, api, name_tmp);
        } else jsonPanicMsg("{s}: unknown type Kind '{s}'", .{ json_basename, kind });
    }
}
...
fn enumerateFunctions(
    ...
    for (functions.items) |*function_node_ptr| {
        ...
        const dll_pool = addStringPoolLower(100, dll_original_case);

        const out_json = try dll_jsons.open(dll_pool);
        ...
        addTypeRef(type_map, json_basename, dll_pool, return_type, .allow_void);
        for (params.items) |*param_node_ptr| {
            const param_obj = param_node_ptr.object;
            try jsonObjEnforceKnownFieldsOnly(param_obj, &[_][]const u8{ "Name", "Type", "Attrs" }, json_basename);
            const param_type = (try jsonObjGetRequired(param_obj, "Type", json_basename)).object;
            addTypeRef(type_map, json_basename, dll_pool, param_type, .no_void);
        }
    }
}
...
fn addTypeRef(
    ...
    const kind = (try jsonObjGetRequired(type_ref, "Kind", json_basename)).string;

    if (std.mem.eql(u8, kind, "Native")) {
        try jsonObjEnforceKnownFieldsOnly(type_ref, &[_][]const u8{ "Kind", "Name" }, json_basename);
        const name = (try jsonObjGetRequired(type_ref, "Name", json_basename)).string;
        ...
        } else if (common.native_type_map.get(name) == null) {
        ...
        if (parents.items.len != 0) {
            std.log.warn(
                ...
                .{std.json.fmt(std.json.Value{.object = type_ref}, .{})},
            );
        } else {
            const api_pool = addStringPoolLower(100, api_tmp);
            const name_pool = global_string_pool.add(name_tmp) catch |e| oom(e);
            const t = getType(type_map, api_pool, name_pool);
            t.dll_set.add(dll);
        }
    } else if (std.mem.eql(u8, kind, "PointerTo")) {
        try jsonObjEnforceKnownFieldsOnly(type_ref, &[_][]const u8{ "Kind", "Child" }, json_basename);
        const child_type_ref = (try jsonObjGetRequired(type_ref, "Child", json_basename)).object;
        addTypeRef(type_map, json_basename, dll, child_type_ref, .no_void);
    } else if (std.mem.eql(u8, kind, "Array")) {
        try jsonObjEnforceKnownFieldsOnly(type_ref, &[_][]const u8{ "Kind", "Shape", "Child" }, json_basename);
        const child_type_ref = (try jsonObjGetRequired(type_ref, "Child", json_basename)).object;
        addTypeRef(type_map, json_basename, dll, child_type_ref, .no_void);
    } else if (std.mem.eql(u8, kind, "LPArray")) {
        try jsonObjEnforceKnownFieldsOnly(type_ref, &[_][]const u8{ "Kind", "NullNullTerm", "CountConst", "CountParamIndex", "Child" }, json_basename);
        const child_type_ref = (try jsonObjGetRequired(type_ref, "Child", json_basename)).object;
        addTypeRef(type_map, json_basename, dll, child_type_ref, .no_void);
    } else if (std.mem.eql(u8, kind, "MissingClrType")) {
        try jsonObjEnforceKnownFieldsOnly(type_ref, &[_][]const u8{ "Kind", "Name", "Namespace" }, json_basename);
        std.log.warn(
            "TODO: add type ref '{}' kind {s}",
            .{std.json.fmt(std.json.Value{.object = type_ref}, .{}), kind},
        );
    } else {
