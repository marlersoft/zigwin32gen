...
fn gatherSdkFiles(sdk_files: *ArrayList(*SdkFile), module: *Module) anyerror!void {
    ...
    std.sort.block(*Module, children, {}, moduleLessThan); // sort so the order is predictable
    for (children) |child| {
...
fn generateContainerModules(dir: std.fs.Dir, module: *Module) anyerror!void {
    ...
    std.sort.block(*Module, children, {}, moduleLessThan);
    if (module.file) |_| {
        try writer.print("//--------------------------------------------------------------------------------\n", .{});
...
fn readAndGenerateApiFile(root_module: *Module, out_dir: std.fs.Dir, json_basename: []const u8, file: std.fs.File) !void {
    ...
    var json_tree = blk: {
        var parser = json.Scanner.initCompleteInput(allocator, content); // false is copy_strings
        defer parser.deinit();
        break :blk try json.Value.jsonParse(allocator, &parser, .{});
    };
    ...
    try generateFile(module_dir, module, json.Parsed(json.Value){ .value = json_tree, .arena = &arena });
...
fn generateFile(module_dir: std.fs.Dir, module: *Module, tree: json.Parsed(json.Value)) !void {
    ...
    }
    {
        ...
        std.sort.block(NamedApiImport, sorted_imports.items, Nothing{}, NamedApiImport.asciiLessThanIgnoreCase);
...
const suppress_enum_aliases = ComptimeStringMapQuota(Nothing, .{
        ...
        switch (attr_node_ptr.*) {
            .object => |attr_obj| {
                ...
                if (std.mem.eql(u8, kind, "MemorySize")) {
                    ...
                    opts.optional_bytes_param_index = @as(i16, @intCast((try jsonObjGetRequired(attr_obj, "BytesParamIndex", sdk_file)).integer));
...
pub fn ComptimeStringMapQuota(comptime V: type, comptime kvs_list: anytype, comptime limit: u32) type {
