...
pub fn main() !u8 {
    ...
    if (cmd_args.len != 5) {
        std.log.err("expected 5 cmdline arguments but got {d}", .{cmd_args.len});
        return 1;
    }
    ...
    const version = blk: {
        ...
        reader.interface.fillMore() catch |err| switch (err) {
            error.EndOfStream => {}, // We read the entire file, which is expected
            else => return err,
        };
        break :blk std.SemanticVersion.parse(reader.interface.buffered()) catch |err|
            fatal("version '{s}' is not a valid semantic version: {t}", .{ reader.interface.buffered(), err });
    };
...

    {
        ...
        std.debug.print("-----------------------------------------------------------------------\n", .{});
        std.debug.print("loading {d} api json files...\n", .{api_list.len});
        ...
        if (global_missing_com_overloads.items.len > 0) {
            std.log.err(
                "missing {d} entries in ComOverloads.txt, copy/paste the following to it:",
                .{global_missing_com_overloads.items.len},
            );
            const stderr = std.fs.File.stderr();
            var err_buf: [128]u8 = undefined;
            var writer = stderr.writer(&err_buf);
...
fn readComOverloads(
    ...
    while (lines.next()) |line| : (line_number += 1) {
        ...
        const com_type = field_it.next() orelse fatal("{s} line {d}: missing type field", .{ filename, line_number });
        const method = field_it.next() orelse fatal("{s} line {d}: missing method field", .{ filename, line_number });
        const method_index_str = field_it.next() orelse fatal("{s} line {d}: missing method index field", .{ filename, line_number });
        const method_index = std.fmt.parseInt(u16, method_index_str, 10) catch |err| fatal(
            "{s} line {d}: invalid method index '{s}': {t}",
            .{ filename, line_number, method_index_str, err },
        );
        const suffix = field_it.next() orelse fatal("{s} line {d}: missing suffix field", .{ filename, line_number });
        if (field_it.next()) |f| fatal(
            "{s} line {d}: has extra field '{s}'",
        ...
        if (suffix_entry.found_existing) fatal(
            "api '{f}' type '{s}' method '{s}' has duplicate entries for index {d}",
...
fn generateEverythingModule(out_win32_dir: std.fs.Dir, root_module: *Module) !void {
    ...
    var out_buf: [output_buf_size]u8 = undefined;
    var file_writer = everything_file.writer(&out_buf);
...
fn generateContainerModules(dir: std.fs.Dir, module: *Module) anyerror!void {
    ...
    if (module.file) |_| {
        try writer.print("//--------------------------------------------------------------------------------\n", .{});
        try writer.print("// Section: SubModules ({d})\n", .{children.len});
        try writer.print("//--------------------------------------------------------------------------------\n", .{});
    } else {
...
fn generateFile(module_dir: std.fs.Dir, module: *Module, api: metadata.Api) !void {
    ...
    defer file_writer.interface.flush() catch @panic("flush failed");
    ...
    try writer.line("");
    try writer.line("//--------------------------------------------------------------------------------");
    try writer.linef("// Section: Functions ({d})", .{api.Functions.len});
    try writer.line("//--------------------------------------------------------------------------------");
    ...
    try writer.line("");
    try writer.line("//--------------------------------------------------------------------------------");
    try writer.linef("// Section: Unicode Aliases ({d})", .{api.UnicodeAliases.len});
    try writer.line("//--------------------------------------------------------------------------------");
    ...
    try writer.line("//--------------------------------------------------------------------------------");
    try writer.linef("// Section: Imports ({d})", .{import_total});
    try writer.line("//--------------------------------------------------------------------------------");
...
fn generateTypeRefRec(
    ...
    switch (self.type_ref) {
        ...
        .Array => |array| {
            ...
            try writer.writef("[{d}]", .{shape_size}, .{ .start = .any, .nl = false });
        ...
        .LPArray => |array| {
            ...
            if (array.NullNullTerm) {
                ...
                if (array.CountConst == -1 or array.CountConst == 0) {
                    ...
                    try writer.writef("*[{d}]", .{array.CountConst}, .{ .start = .any, .nl = false });
                }
...
fn generateConstant(sdk_file: *SdkFile, writer: *CodeWriter, constant: metadata.Constant) !void {
    ...
    if (constant.Type == .Native) {
        ...
        if (constant.ValueType == .PropertyKey) {
            ...
            try writer.writef(" {{ .fmtid = Guid.initString(\"{s}\"), .pid = {d} }};", .{ fmtid, pid }, .{ .start = .mid });
        } else {
...
fn generateType(
    ...
    if (types_that_conflict_with_consts.get(t.Name)) |_| {
        ...
        const def_prefix = try std.fmt.allocPrint(allocator, "pub const {f} = ", .{std.zig.fmtIdP(t.Name)});
    ...
    switch (t.Kind) {
        .NativeTypedef => |typedef| {
            ...
            if (typedef.InvalidHandleValue) |v| {
                try writer.linef("// TODO: this type has an InvalidHandleValue of '{d}', what can Zig do with this information?", .{v});
            }
...
fn generateStructOrUnionDef(
    ...
    for (container.NestedTypes) |*nested_type| {
        ...
        if (isAnonymousTypeName(nested_type.Name)) {
            ...
            //const def_prefix = try std.fmt.allocPrint(allocator, "pub const {f} = ", .{std.zig.fmtIdP(pool_name)});
            //defer allocator.free(def_prefix);
    ...
    if (container.Fields.len == 0) {
        ...
        for (container.Fields) |*field| {
            ...
            try writer.writef("{f}: ", .{std.zig.fmtIdP(field.Name)}, .{ .nl = false });
            try generateTypeRef(sdk_file, writer, field_type_formatter);
            if (container.PackingSize >= 1) {
                try writer.writef(" align({d})", .{container.PackingSize}, .{ .start = .mid, .nl = false });
            }
...
fn generateEnum(
    ...
    if (values.len == 0) {
        ...
        for (values) |val| {
            if (val.conflict_index == null) {
                try writer.linef("    {f} = {f},", .{ std.zig.fmtIdP(val.short_name), fmtJson(val.value) });
            }
        }
        ...
        for (values) |val| {
            if (val.conflict_index) |conflict_index| {
                try writer.linef("    pub const {f} = .{f};", .{ std.zig.fmtIdP(val.short_name), std.zig.fmtIdP(values[conflict_index].short_name) });
            }
        }
        if (non_exhaustive_enums.get(pool_name.slice)) |_| {
            writer.depth += 1;
    ...
    if (type_enum.Flags) {
        ...
        for (flag_map[0..bit_count], 0..) |maybe_flag, i| {
            ...
            try writer.linef("    {f}: u1 = 0,", .{std.zig.fmtIdP(flag.short_name)});
        }
        ...
        for (values) |*val| {
            switch (val.flagsValue()) {
                ...
                .flag => |index| {
                    if (val.conflict_index) |conflict_index| {
                        try writer.linef("    // {f} (bit index {d}) conflicts with {f}", .{
                            std.zig.fmtIdP(val.short_name),
                            index,
                            std.zig.fmtIdP(values[conflict_index].short_name),
                        });
                    }
                },
                .mask => {},
    ...
    for (values) |val| {
        ...
        if (type_enum.Flags) switch (val.flagsValue()) {
            ...
            .flag => try writer.linef("pub const {f} = {f}{{ .{f} = 1 }};", .{ val.pool_name, pool_name, std.zig.fmtIdP(target_short_name) }),
            .mask => |mask| {
                ...
                while (true) : (index += 1) {
                    ...
                    if (0 != (next_flag_bit & mask)) {
                        if (flag_map[index]) |flag| {
                            ...
                            try writer.linef("    .{f} = 1,", .{std.zig.fmtIdP(flag_target_short_name)});
                        } else {
                            try writer.linef("    ._{d} = 1,", .{index});
                        }
            ...
            try writer.linef("pub const {f} = {f}.{f};", .{ val.pool_name, pool_name, std.zig.fmtIdP(target_short_name) });
        }
    }
}
        ...
        for (type_com.Methods, 0..) |*method, method_index| {
            ...
            const suffix: []const u8 = blk: {
                ...
                break :blk suffixes.get(@intCast(method_index)) orelse fatal(
                    "method '{s}' is missing suffix for method at index {d}",
...
const ConflictSuffix = struct {
    ...
    pub fn format(self: ConflictSuffix, writer: *std.Io.Writer) !void {
        if (self.conflict_count > 0) {
            try writer.print("{d}", .{self.conflict_count});
        }
    }
};
...
fn generateParams(
    ...
    for (params, 0..) |*param, i| {
        ...
        if (param_options.optional_bytes_param_index) |bytes_param_index| {
            try writer.linef("    // TODO: what to do with BytesParamIndex {d}?", .{bytes_param_index});
        }
        try writer.writef("    {f}: ", .{std.zig.fmtIdP(param.Name)}, .{ .nl = false });
        try generateTypeRef(sdk_file, writer, param_type_formatter);
        try writer.write(",", .{ .start = .mid });
    }
}
