...
const SdkFile = struct {
    ...
    includes: StringHashMap(Nothing),
    ...
    pub fn create(json_basename: []const u8) !*SdkFile {
        ...
        sdk_file.* = .{
            ...
            .includes = StringHashMap(Nothing).init(allocator),
...
fn main2() !u8 {
    ...
    var api_json_dir = windows_api_dir.openDir(api_json_sub_path, .{.iterate = true}) catch |e| switch (e) {
        error.FileNotFound => {
            std.debug.warn("Error: JSON files have not been generated in '{s}{s}{s}', generate them with:\n", .{windows_api_dir_name, path_sep, api_json_sub_path});
            std.debug.warn("    python3 {s}" ++ path_sep ++ windows_api_dir_name ++ path_sep ++ "json-gen\n", .{try getcwd(allocator)});
            return error.AlreadyReported;
        },
        else => return e,
    };
    ...
    // copy glue.zig module
    {
        var src_dir = try cwd.openDir("src", .{});
        defer src_dir.close();
        try src_dir.copyFile("glue.zig", out_windows_dir, "glue.zig", .{});
    }
    ...
    defer sdk_files.deinit();
    {
        ...
        for (sdk_files.items) |sdk_file| {
            ...
            for (sdk_file.type_imports.items) |type_name| {
                if (shared_type_export_map.get(type_name)) |entry| {
                    if (sdk_file.includes.get(entry.first_sdk_file_ptr.name)) |_| {
                        try writer.print("// {s} included by pub usingnamespace of {s}.zig\n", .{type_name, entry.first_sdk_file_ptr.name});
                    } else {
                        try writer.print("const {s} = @import(\"./{s}.zig\").{s};\n", .{type_name, entry.first_sdk_file_ptr.name, type_name});
                    }
                } else {
                    // TODO: uncomment this warning after all types start being generated
                    //std.debug.warn("WARNING: module '{s}' uses undefined type '{s}'\n", .{ sdk_file.name, type_name});
                    try writer.print("const {s} = c_int; // WARNING: this is a placeholder because this type is undefined\n", .{type_name});
                }
            }
        }
        {
            ...
            for (sdk_files.items) |sdk_file| {
                try writer.print("pub const {s} = @import(\"./header/{s}.zig\");\n", .{sdk_file.name, sdk_file.name});
            }
            try writer.writeAll(
                \\const std = @import("std");
                \\test "" {
...

    {
        ...
        try writer.writeAll(
            \\//! This file is autogenerated
            \\pub const glue = @import("./windows/glue.zig");
...
fn generateFile(out_dir: std.fs.Dir, func_dll_map: StringHashMap(*FuncDllEntry), sdk_file: *SdkFile, tree: json.ValueTree) !void {
    ...
    for (includes_array.items) |include_node| {
        try generateInclude(sdk_file, out_writer, include_node.Object);
    }
    try out_writer.print("//\n", .{});
    try out_writer.print("// {} types\n", .{types_array.items.len});
    try out_writer.print("//\n", .{});
    for (types_array.items) |type_node| {
        try generateTopLevelType(sdk_file, out_writer, type_node.Object);
    }
    try out_writer.print("//\n", .{});
...
fn generateInclude(sdk_file: *SdkFile, out_writer: std.fs.File.Writer, include_obj: json.ObjectMap) !void {
    try jsonObjEnforceKnownFieldsOnly(include_obj, &[_][]const u8 {"filename"}, sdk_file);
    const filename_tmp = (try jsonObjGetRequired(include_obj, "filename", sdk_file)).String;
    try out_writer.print("pub usingnamespace @import(\"./{s}.zig\");\n", .{filename_tmp});
    try sdk_file.includes.put(try global_symbol_pool.add(filename_tmp), .{});
}
...
fn generateFunction(func_dll_map: StringHashMap(*FuncDllEntry), sdk_file: *SdkFile, out_writer: std.fs.File.Writer, function_obj: json.ObjectMap) !void {
    ...
    const func_entry = func_dll_map.get(func_name_pool) orelse {
        std.debug.warn("Error: function '{s}' is not in any dll json file\n", .{func_name_pool});
        return error.AlreadyReported;
    };
    if (func_entry.dlls.items.len != 1) {
        std.debug.warn("Error: function '{s}' is found in these {} dlls:\n", .{func_name_pool, func_entry.dlls.items.len});
...
fn generateUnicodeName(sdk_file: *SdkFile, out_writer: std.fs.File.Writer, unicode_name_obj: json.ObjectMap) !void {
    ...
    try out_writer.print("pub const {s} = switch (@import(\"../glue.zig\").unicode_mode) {{\n", .{name_pool});
    if (ansi_kind == .none) {
