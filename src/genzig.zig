...
const SdkFile = struct {
    ...
    api_type_refs: StringPool.HashMap(ApiRef),
    ...
    type_imports: ArrayList(StringPool.Val),
    ...
    pub fn create(json_basename: []const u8) !*SdkFile {
        ...
        sdk_file.* = .{
            ...
            .api_type_refs = StringPool.HashMap(ApiRef).init(allocator),
            .type_exports = StringPool.HashMap(Nothing).init(allocator),
            .func_exports = StringPool.HashMap(Nothing).init(allocator),
            .type_imports = ArrayList(StringPool.Val).init(allocator),
        };
        return sdk_file;
    }

    pub fn addApiRef(self: *SdkFile, name: []const u8, api: []const u8, parents: json.Array) !void {
        const pool_name = try global_symbol_pool.add(name);
        if (self.api_type_refs.get(pool_name)) |other| {
            std.debug.panic("there are multiple type references to '{s}' in the same api", .{name});
        }
        const pool_api = try global_symbol_pool.add(api);
        const pool_parents = if (parents.items.len == 0) &[_]StringPool.Val{} else
            try allocator.alloc(StringPool.Val, parents.items.len);
        for (parents.items) |p,i| {
            pool_parents[i] = try global_symbol_pool.add(p.String);
        }
        try self.api_type_refs.put(pool_name, .{
            .name = pool_name,
            .api = pool_api,
...
fn main2() !u8 {
    ...
    defer sdk_files.deinit();
    {
        ...
        for (sdk_files.items) |sdk_file| {
            var out_file = try out_api_dir.openFile(sdk_file.zig_filename, .{.read = false, .write = true});
            ...
            for (sdk_file.type_imports.items) |type_name| {
                if (shared_type_export_map.get(type_name)) |entry| {
                    try writer.print("const {s} = @import(\"{s}.zig\").{0s};\n", .{type_name, entry.first_sdk_file_ptr.name_snake_case});
                } else {
                    // TODO: uncomment this warning after all types start being generated
...
fn generateFile(out_dir: std.fs.Dir, sdk_file: *SdkFile, tree: json.ValueTree) !void {
    ...
    , .{sdk_file.const_exports.items.len,
        ...
        sdk_file.type_imports.items.len,
    });
}
...
fn addTypeRefsNoFormatter(sdk_file: *SdkFile, type_ref: json.ObjectMap) anyerror!void {
    ...
    if (std.mem.eql(u8, kind, "Native")) {
        ...
        try sdk_file.addApiRef(tmp_name, api, parents);
    ...
    if (std.mem.eql(u8, kind, "NativeTypedef")) {
        ...
        if (struct_fields.items.len == 0) {
            ...
            for (struct_fields.items) |field_node| {
                ...
                try out_writer.print("    {}: u32, // TODO: type ref {}\n", .{std.zig.fmtId(field_name), fmtJson(field_type)});
            }
            try out_writer.print("}};\n", .{});
        }
    } else {
