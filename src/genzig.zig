...
const global = struct {
    ...
    var symbol_pool = StringPool.init(arena);
    const symbol = struct {
        var none: CachedSymbol("none") = .{};
        var None: CachedSymbol("None") = .{};
        var WIN32_ERROR: CachedSymbol("WIN32_ERROR") = .{};
    };
};

const TypeIndex = std.HashMapUnmanaged(
    TypeKey,
    *const metadata.Type,
    TypeKey.Context,
...
const Gen = struct {
    const Mutable = struct {
        ...
        found_win32_error: bool = false,
        dll_modules: StringPool.HashMapUnmanaged(*DllModule) = .{},
        missing_com_overloads: std.ArrayListUnmanaged(MissingOverload) = .{},
    };
    mut: *Mutable,
    root_module: *Module,
    extra_api_map: *const extra.Root,
...
fn CachedSymbol(comptime value: [:0]const u8) type {
    return struct {
        cached: ?StringPool.Val = null,
        pub fn get(symbol: *@This()) StringPool.Val {
            if (symbol.cached == null) {
                symbol.cached = global.symbol_pool.add(value) catch |e| oom(e);
            }
...
const Module = struct {
    ...
    pub fn alloc(optional_parent: ?*Module, name: StringPool.Val) !*Module {
        const module = try global.arena.create(Module);
        module.* = Module{
            .optional_parent = optional_parent,
            .name = name,
            .zig_basename = try std.mem.concat(global.arena, u8, &[_][]const u8{ name.slice, ".zig" }),
            .children = StringPool.HashMap(*Module).init(global.arena),
            .file = null,
        };
        return module;
    }
...
const SdkFile = struct {
    ...
    pub fn addApiImport(
        ...
        const top_level_symbol = try global.symbol_pool.add(if (parents.len == 0) name else parents[0]);
...
pub fn main() !u8 {
    ...
    defer {
        if (print_time_summary) {
            ...
            std.debug.print("Gen Time  : {} millis ({}%)\n", .{ mut.generate_time_millis, @divTrunc(100 * mut.generate_time_millis, total_millis) });
    ...
    const text = blk: {
        ...
        break :blk try reader.interface.readAlloc(global.arena, @intCast(size));
    };
    const parsed = textparse.parseAll(global.arena, text);

    const api_list: []StringPool.Val = blk: {
        var api_list = std.array_list.Managed(StringPool.Val).init(global.arena);
        for (parsed) |named| {
            try api_list.append(try global.symbol_pool.add(named.name));
        }
    ...
    var type_index: TypeIndex = .{};
    for (parsed) |named| {
        try api_by_name.put(global.arena, named.name, named.api);
        for (named.api.Types) |*t| {
            try type_index.put(global.arena, .{ .api = named.name, .name = t.Name }, t);
        }
    }
    const apis = try global.arena.alloc(metadata.Api, api_list.len);
    ...
    const pass1_root = blk: {
        const names = try global.arena.alloc([]const u8, api_list.len);
        for (api_list, names) |a, *n| n.* = a.slice;
        break :blk pass1.buildIndex(global.arena, names, apis);
    };

    const api_set = blk: {
        var api_set: StringPool.HashMapUnmanaged(void) = .{};
        for (api_list) |api| {
            try api_set.putNoClobber(global.arena, api, {});
        }
        break :blk api_set;
    };
    ...
    const extra_api_map = extra.read(api_set, &global.symbol_pool, global.arena, extra_filename, extra_content);
    // no need to free
    ...
    const gen: Gen = .{
        .root_module = try Module.alloc(null, try global.symbol_pool.add("win32")),
        .extra_api_map = &extra_api_map,
...

    {
        ...
        for (api_list, apis, 0..) |api_name, api, api_index| {
            ...
            try generateApiModule(&gen, out_win32_dir, api_name, api);
        }
        std.debug.assert(gen.mut.found_win32_error);

        if (gen.mut.missing_com_overloads.items.len > 0) {
            std.log.err(
                "missing {} entries in ComOverloads.txt, copy/paste the following to it:",
                .{gen.mut.missing_com_overloads.items.len},
            );
            var stderr_buf: [4096]u8 = undefined;
            var stderr = std.fs.File.stderr().writer(&stderr_buf);
            for (gen.mut.missing_com_overloads.items) |overload| {
        ...
        for (static_zig_files ++ &[_][]const u8{
            ...
            const submodule = try global.symbol_pool.add(submodule_str);
            try gen.root_module.children.put(submodule, try Module.alloc(gen.root_module, submodule));
        }

        try writeDllModules(&gen.mut.dll_modules, out_dir);

        try generateContainerModules(&gen.mut.dll_modules, out_dir, gen.root_module);
        try generateEverythingModule(&gen, out_win32_dir);
    }
...
fn readComOverloads(
    ...
    while (lines.next()) |line| : (line_number += 1) {
        ...
        const api = try global.symbol_pool.add(field_it.next() orelse continue);
        ...
        const api_entry = try api_map.getOrPut(global.arena, api);
        if (!api_entry.found_existing) {
            api_entry.value_ptr.* = .{};
        }
        const type_map = api_entry.value_ptr;
        const type_entry = try type_map.getOrPut(global.arena, com_type);
        if (!type_entry.found_existing) {
            type_entry.value_ptr.* = .{};
        }
        const method_map = type_entry.value_ptr;
        const method_entry = try method_map.getOrPut(global.arena, method);
        if (!method_entry.found_existing) {
            method_entry.value_ptr.* = .{};
        }
        const suffix_map = method_entry.value_ptr;
        const suffix_entry = try suffix_map.getOrPut(global.arena, method_index);
...
fn gatherSdkFiles(sdk_files: *std.array_list.Managed(*SdkFile), module: *Module) anyerror!void {
    ...
    const children = try common.allocMapValues(global.arena, *Module, module.children);
    defer global.arena.free(children);
...
fn generateEverythingModule(gen: *const Gen, out_win32_dir: std.fs.Dir) !void {
    ...
    {
        var dll_names = std.array_list.Managed(StringPool.Val).init(global.arena);
        ...
        for (dll_names.items) |name| {
            const dm = gen.mut.dll_modules.get(name).?;
    ...
    var sdk_files = std.array_list.Managed(*SdkFile).init(global.arena);
...
fn generateContainerModules(
    dll_modules: *const StringPool.HashMapUnmanaged(*DllModule),
    dir: std.fs.Dir,
    module: *Module,
) anyerror!void {
    ...
    for (children) |child| {
        if (isDllModule(dll_modules, child)) dll_count += 1;
    }
    ...
    for (children) |child| {
        try generateContainerModules(dll_modules, next_dir, child);
    }
...
fn generateApiModule(
    ...
    const zig_name = try cameltosnake.camelToSnakeAlloc(global.arena, api_name.slice);
    errdefer global.arena.free(zig_name);

    var module_dir = out_dir;
    defer if (module_dir.fd != out_dir.fd) module_dir.close();

    var module: *Module = gen.root_module;
    var depth: u2 = 0;

    {
        var it = std.mem.tokenizeScalar(u8, zig_name, '.');
        while (it.next()) |name_part| {
            if (module != gen.root_module) {
                depth += 1;
                if (module.children.count() == 0) {
            ...
            const name_pool = try global.symbol_pool.add(name_part);
    ...
    if (gen.extra_api_map.get(api_name)) |api_obj| {
    ...
    module.file = SdkFile{
        ...
        .uses_guid = false,
        .top_level_api_imports = StringPool.HashMap(ApiImport).init(global.arena),
        .type_exports = StringPoolArrayHashMap(void).init(global.arena),
        .func_exports = StringPoolArrayHashMap(void).init(global.arena),
        .tmp_func_ptr_workaround_list = std.array_list.Managed(StringPool.Val).init(global.arena),
        .method_conflict_map = getMethodConflictMap(api_name.slice),
        .param_conflict_map = getParamConflictMap(api_name.slice),
        .extra_funcs = extra_funcs,
        .extra_funcs_applied = StringPool.HashMap(void).init(global.arena),
        .extra_consts = extra_consts,
        .extra_consts_applied = StringPool.HashMap(void).init(global.arena),
        .com_type_overloads = gen.com_overload_map.get(api_name),
    };

    const generate_start_millis = std.time.milliTimestamp();
    try generateFile(gen, module_dir, module, api_root);
    gen.mut.generate_time_millis += std.time.milliTimestamp() - generate_start_millis;
}
...
fn isDllModule(dll_modules: *const StringPool.HashMapUnmanaged(*DllModule), module: *Module) bool {
...
fn writeDllModules(dll_modules: *const StringPool.HashMapUnmanaged(*DllModule), out_dir: std.fs.Dir) !void {
...
fn writeImportsSection(sdk_file: *SdkFile, writer: *CodeWriter) !void {
    ...
    {
        var arch_specific_imports = ArchSpecificMap(StringPool.Val).init(global.arena);
        ...
        var sorted_imports = std.array_list.Managed(NamedApiImport).init(global.arena);
        ...
        for (sorted_imports.items) |import| {
            ...
            if (redirects.get(import.name.slice)) |target| switch (target) {
                ...
                defer global.arena.free(api_path);
...
fn generateFile(gen: *const Gen, module_dir: std.fs.Dir, module: *Module, api: metadata.Api) !void {
    ...
    for (api.Constants) |constant| {
        try generateConstant(gen, sdk_file, writer, constant);
    }
    ...
    try writer.line("//--------------------------------------------------------------------------------");
    {
        var arch_specific_types = ArchSpecificMap(metadata.Type).init(global.arena);
        defer arch_specific_types.deinit();
        var enum_alias_conflicts = StringPool.HashMap(StringPool.Val).init(global.arena);
        defer enum_alias_conflicts.deinit();
        for (api.Types) |t| {
            try generateType(gen, sdk_file, writer, &arch_specific_types, t, &enum_alias_conflicts);
            try writer.line("");
        }
        ...
        while (it.next()) |entry| {
            ...
            for (entry.value_ptr.getItemsConst()) |object| {
                ...
                try generateTypeDefinition(gen, sdk_file, writer, object.obj, &enum_alias_conflicts, name_pool, def_prefix, ",");
            }
            if (combined_arches.filter != null) {
                //try writer.line("    else => @compileError(\"unsupported on this arch\"),");
                try writer.line("    else => usize, // NOTE: this should be a @compileError but can't because of https://github.com/ziglang/zig/issues/9682");
    ...
    for (api.Functions) |*function| {
        if (function.Architectures.filter != null) {
            const name_pool = try global.symbol_pool.add(function.Name);
            const entry = arch_func_map.getOrPut(global.arena, name_pool) catch |e| oom(e);
            const new_func_node_index: FuncNodeIndex = @enumFromInt(arch_func_nodes.items.len);
            arch_func_nodes.append(global.arena, .{
                .func = function,
    ...
    for (api.Functions) |function| {
        ...
        const dm = getDllModule(gen, function.DllImport);
        ...
        var aw = std.Io.Writer.Allocating.init(global.arena);
        var cw = CodeWriter{ .writer = &aw.writer, .depth = 0, .midline = false };
        const dw = &cw;

        if (function.Architectures.filter == null) {
            try generateFunction(gen, dsf, sdk_file, dw, .{ .dll = function });
        } else {
            ...
            while (true) {
                ...
                try dw.line("");
                try generateFunction(gen, dsf, sdk_file, dw, .{ .dll = node.func.* });
                try dw.line("");
        ...
        try dw.line("");
        dm.funcs.append(global.arena, .{ .name = name_pool, .text = global.arena.dupe(u8, aw.written()) catch |e| oom(e) }) catch |e| oom(e);
    }
...
fn allocApiImportPathFromRef(api_ref: []const u8) ![]u8 {
    const api_path = try cameltosnake.camelToSnakeAlloc(global.arena, api_ref);
    for (api_path, 0..) |c, i| {
        if (c == '.')
            api_path[i] = '/';
    }
...
fn addTypeRefsNoFormatter(sdk_file: *SdkFile, arches: metadata.Architectures, type_ref: metadata.TypeRef) anyerror!void {
    switch (type_ref) {
        ...
        .ApiRef => |api_ref| {
            ...
            const api = try global.symbol_pool.add(api_ref.Api);
...
fn generateTypeRef(
    ...
    try generateTypeRefRec(gen, sdk_file, writer, self, .top_level);
}
    ...
    switch (self.type_ref) {
        ...
        .ApiRef => |api_ref| {
            ...
            if (isAnonymousTypeName(name)) {
                ...
                const name_pool = try global.symbol_pool.add(name);
                const t = anon_types.types.get(name_pool) orelse
                    failMsg("missing anonymous type '{f}'!", .{name_pool});
                switch (t.Kind) {
                    .Struct => try generateStructOrUnionDef(gen, sdk_file, writer, t, self.nested_context),
                    .Union => try generateStructOrUnionDef(gen, sdk_file, writer, t, self.nested_context),
                    else => fail(),
                }
            ...
            const type_kind_category: Pass1TypeCategory = blk: {
                const pass1_api_map = gen.pass1.get(api_ref.Api) orelse failMsg("type '{s}' is from API '{s}' that is missing from pass1 data", .{ name, api_ref.Api });
        ...
        .PointerTo => |to| {
            ...
            try generateTypeRefRec(gen, sdk_file, writer, fmtTypeRef(to.Child.*, self.arches, child_options, self.nested_context), .child);
        },
        .Array => |array| {
            ...
            try generateTypeRefRec(gen, sdk_file, writer, fmtTypeRef(array.Child.*, self.arches, self.options, self.nested_context), .child);
        },
        .LPArray => |array| {
            ...
            if (array.NullNullTerm) {
                ...
                if (array.CountConst <= 0 and self.options.is_const)
                    try writer.write("const ", .{ .start = .any, .nl = false });
                try generateTypeRefRec(gen, sdk_file, writer, fmtTypeRef(array.Child.*, self.arches, self.options.getChildOptions(), self.nested_context), .array);
            }
        },
...
fn resolveInitStruct(gen: *const Gen, api: []const u8, name: []const u8) *const metadata.StructOrUnion {
    const t = gen.type_index.get(.{ .api = api, .name = name }) orelse
...
fn generateConstant(gen: *const Gen, sdk_file: *SdkFile, writer: *CodeWriter, constant: metadata.Constant) !void {
    const name_pool = try global.symbol_pool.add(constant.Name);
    ...
    switch (constant.Value) {
        .initializer => |init_str| {
            ...
            const su = resolveInitStruct(gen, api_ref.Api, api_ref.Name);
            var nums = InitNumbers.init(init_str);
            try writer.writef("pub const {f} = ", .{name_pool}, .{ .nl = false });
            try generateTypeRef(gen, sdk_file, writer, zig_type_formatter);
...
fn generateType(
    ...
    switch (t.Kind) {
        .ComClassID => |class_id| {
            if (t.Architectures.filter != null)
                failMsg("not impl", .{});
            const clsid_pool = try global.symbol_pool.addFormatted("CLSID_{s}", .{t.Name});
            sdk_file.uses_guid = true;
    ...
    const pool_name = try global.symbol_pool.add(renameType(t.Name));
    ...
    if (types_that_conflict_with_consts.get(t.Name)) |_| {
        ...
        const def_prefix = try std.fmt.allocPrint(global.arena, "pub const {f} = ", .{fmtIdP(renameType(t.Name))});
        defer global.arena.free(def_prefix);
        try generateTypeDefinition(gen, sdk_file, writer, t, enum_alias_conflicts, pool_name, def_prefix, ";");
    }
}
...
fn generateTypeDefinition(
    ...
    switch (t.Kind) {
        .NativeTypedef => |typedef| {
            ...
            if (typedef.AlsoUsableFor) |also_usable_for| {
                if (also_usable_type_api_map.get(also_usable_for)) |api| {
                    const api_pool = try global.symbol_pool.add(api);
        ...
        .Enum => |type_enum| try generateEnum(gen, sdk_file, writer, type_enum, pool_name, enum_alias_conflicts, def_prefix, def_suffix),
        .Struct => try generateStructOrUnion(gen, sdk_file, writer, t, def_prefix, def_suffix, null),
        .Union => try generateStructOrUnion(gen, sdk_file, writer, t, def_prefix, def_suffix, null),
        .ComClassID => @panic("hasn't happened yet?"),
        .Com => |com| try generateCom(gen, sdk_file, writer, t, com, pool_name, def_prefix),
        .FunctionPointer => |func| {
            ...
            try generateFunction(gen, sdk_file, sdk_file, writer, .{ .ptr = .{
                .t = t,
                .func = func,
...
const redirects = std.StaticStringMap(Redirect).initComptime(.{
    ...
    .{ "NTSTATUS", Redirect{ .native = "@import(\"std\").os.windows.NTSTATUS" } },
});
...
const AnonTypes = struct {
    ...
    pub fn init() AnonTypes {
        return .{
            .types = StringPool.HashMap(metadata.Type).init(global.arena),
        };
    }
...
fn generateStructOrUnion(
    ...
    try writer.writef("{s}", .{def_prefix}, .{ .nl = false });
    try generateStructOrUnionDef(gen, sdk_file, writer, t, nested_context);
...
fn generateStructOrUnionDef(
    ...
    for (container.NestedTypes) |*nested_type| {
        const pool_name = try global.symbol_pool.add(nested_type.Name);
        if (isAnonymousTypeName(nested_type.Name)) {
            ...
            //const def_prefix = try std.fmt.allocPrint(global.arena, "pub const {f} = ", .{fmtIdP(pool_name)});
            //defer global.arena.free(def_prefix);
            try writer.writef("pub const {f} = ", .{pool_name}, .{ .nl = false });
            switch (nested_type.Kind) {
                .Union => try generateStructOrUnionDef(gen, sdk_file, writer, nested_type.*, this_nested_context),
                .Struct => try generateStructOrUnionDef(gen, sdk_file, writer, nested_type.*, this_nested_context),
    ...
    if (container.Fields.len == 0) {
        ...
        for (container.Fields) |*field| {
            ...
            try generateTypeRef(gen, sdk_file, writer, field_type_formatter);
...
fn setShortNames(values: []EnumValue) void {
    ...
    for (values) |*val_ref| {
        if ((val_ref.pool_name.eql(global.symbol.none.get()) or
            val_ref.pool_name.eql(global.symbol.None.get())) and
            val_ref.valueIsZero())
        {
...
fn generateEnum(
    ...
    const values = try global.arena.alloc(EnumValue, type_enum.Values.len);
    var values_initialized_len: usize = 0;
    defer global.arena.free(values);
    for (type_enum.Values) |*enum_field| {
        values[values_initialized_len] = .{
            .pool_name = try global.symbol_pool.add(enum_field.Name),
            .short_name = "",
    ...
    if (values.len == 0) {
        ...
        if (non_exhaustive_enums.get(pool_name.slice)) |_| {
            ...
            try writer.line("    };");
            try writer.line("}");
            const is_win32_error = pool_name.eql(global.symbol.WIN32_ERROR.get());
            if (is_win32_error) {
                std.debug.assert(!gen.mut.found_win32_error);
                gen.mut.found_win32_error = true;
...
fn getComInterface(gen: *const Gen, api: []const u8, name: []const u8) ?metadata.TypeRef {
    const pass1_api_map = gen.pass1.get(api) orelse failMsg("com interface inside unknown api '{s}'", .{api});
...
fn generateCom(
    ...
    const iid_pool = try global.symbol_pool.addFormatted("IID_{s}", .{com_pool_name.slice});
...

    {
        ...
        var method_set = StringHashMap(ComSymbolState).init(global.arena);
        ...
        const count_before = gen.mut.missing_com_overloads.items.len;
        for (type_com.Methods, 0..) |*method, method_index| {
            ...
            switch (state) {
                ...
                .conflicts => {
                    ...
                    if (!have_overload) {
                        try gen.mut.missing_com_overloads.append(global.arena, .{
                            .api = sdk_file.api_name,
                            .com_type = com_pool_name.slice,
                            .method = try global.arena.dupe(u8, method.Name),
        ...
        if (count_before != gen.mut.missing_com_overloads.items.len)
            return;
    }
    ...
    if (skip) {
        ...
        if (type_com.Interface) |iface| {
            ...
            try generateTypeRef(gen, sdk_file, writer, maybe_iface_formatter.?);
        ...
        for (type_com.Methods, 0..) |*method, method_index| {
            ...
            try generateFunction(gen, sdk_file, sdk_file, writer, .{ .com = .{
                .method = method,
    ...
    if (type_com.Interface) |direct_iface| {
        ...
        while (true) {
            ...
            try writer.write("    ", .{ .nl = false });
            try generateTypeRef(gen, sdk_file, writer, iface_formatter);
            try writer.write(": ", .{ .start = .mid, .nl = false });
            try generateTypeRef(gen, sdk_file, writer, iface_formatter);
            try writer.write(",", .{ .start = .mid });

            next_iface = getComInterface(gen, next_iface_com.api, next_iface_com.name) orelse break;
...
fn generateComMethods(
    ...
    for (com_methods, 0..) |*method, method_index| {
        ...
        for (method.Params, 0..) |*param, param_index| {
            ...
            try generateTypeRef(gen, sdk_file, writer, param_type_formatter);
        }
        // NOTE: don't need to call addTypeRefs because it was already called in generateFunction above
        // TODO: set is_const, in and out properly
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        try generateTypeRef(gen, sdk_file, writer, return_type_formatter);
        try writer.write(" {", .{ .start = .mid });
...
fn getComMethodConfigName(type_name: []const u8, method: *const metadata.ComMethod) StringPool.Val {
    ...
    return global.symbol_pool.add(name) catch |e| oom(e);
}
...
const Function = union(enum) {
    ...
    pub fn ConfigName(self: Function) StringPool.Val {
        return switch (self) {
            .dll => |dll| global.symbol_pool.add(dll.Name) catch |e| oom(e),
            .com => |com| return getComMethodConfigName(com.type_name, com.method),
            .ptr => |ptr| global.symbol_pool.add(ptr.t.Name) catch |e| oom(e),
        };
    }
...
fn comMethodReturnsStructByValue(gen: *const Gen, return_type: metadata.TypeRef) bool {
    switch (return_type) {
        .ApiRef => |api_ref| {
            if (api_ref.TargetKind != .Default) return false;
            const type_map = gen.pass1.get(api_ref.Api) orelse return false;
...
fn generateFunction(
    ...
    switch (func) {
        .dll => |dll| sdk_file.func_exports.put(try global.symbol_pool.add(dll.Name), {}) catch |e| oom(e),
        .com => {},
        .ptr => {},
    }
    ...
    const is_com_struct_return = switch (func) {
        .com => comMethodReturnsStructByValue(gen, return_type),
        else => false,
    };
    ...
    try generateParams(gen, sdk_file, writer, arches, modifier_set, params);
